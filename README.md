...
> 불필요한 네트워크/스레드 오버헤드를 줄여 **단일 서버에서 초당 평균 약 89,500건(1초 피크 94,110건)의 웹소켓 메시지를 안정적으로 처리**하도록 최적화했습니다.
...
3. **검증 가능한 성능 수치** — WS 동접 10,000에서 sustain 평균 89.5k / 1초 피크 94.1k msg/s (서버 실측), durable 커맨드 로그·스냅샷을 켠 상태에서도 -3.5%. 계측 설계와 측정 한계까지 [방법론](#측정-방법론)으로 공개
 ...
 **결과:** I/O 병목을 해소하여 단일 서버에서 **초당 평균 약 89,500건(1초 피크 94,110건)** 의 웹소켓 메시지를 처리합니다([부하 테스트 결과](#-부하-테스트-결과) 참조). 전환 시점의 동일 부하 시나리오 비교 측정에서는 Redis 프로파일 대비 **약 6.6배** 의 처리량 향상을 확인했습니다.
...
### 결과 (2026-08 재측정, 전 thresholds 통과)

| 지표 | 값 |
| :--- | :--- |
| 초당 WS 송신 메시지 — sustain 평균 (서버 실측) | **약 89,500 msg/s** (중앙값 92,571) |
| 초당 WS 송신 메시지 — 1초 피크 (서버 실측) | **94,110 msg/s** |
| 초당 WS 메시지 (k6 수신, 10.5분 전체 평균) | 약 71,200 msg/s |
| 총 송수신 메시지 | 약 52,790,000 |
| **게임 액션 RTT** avg / med / P95 / P99 / max | **10.3ms / 1ms / 48ms / 114ms / 3,788ms** |
| 완주된 게임 수 | 225,604 판 (약 358 games/s) |
| 서버 에러 / 무응답 타임아웃 | **0건 / 0건** (checks 100%) |
| WS Handshake P95 / P99 / max | 2.98ms / 6.78ms / 54.2ms |
